...
% BinSize gives the size of a bin in input units (i.e. not scaled by
% SampleRate)
%                          
%%%  Error !  scaled by SampleRate ! 
...
if isempty(Epochs)
	...
	for e=1:size(Epochs,1)
        ...
        %Original line in Ken's code. 
        %EpochBias = clip(EpochLen - nTerm*BinSize,0,inf)*BinSize;
        %New line added by Alfonso (Dec5 2011). Uses diff. syntax for clip
        %function
        EpochBias = clip(EpochLen - nTerm*BinSize,[0 inf])*BinSize;
        Bias = Bias+EpochBias';
