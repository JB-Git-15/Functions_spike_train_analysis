...
/*		mexPrintf("Allocating pair memory\n"); */
        ...
        if (!Pairs) mexErrMsgIdAndTxt("001","Could not allocate memory for pairs");
	}
...
*/
        ...
        mexErrMsgIdAndTxt("002","Too many pairs");
	}
	Pairs[PairCnt++] = n1;
	Pairs[PairCnt++] = n2;
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ) {
	...
	if (mxGetClassID(prhs[0])!=mxDOUBLE_CLASS || mxGetClassID(prhs[1])!=mxUINT32_CLASS
		...
		mexErrMsgIdAndTxt("004","Arguments are wrong type\n");
	}
	...
	if (mxGetNumberOfElements(prhs[1])!=nSpikes) mexErrMsgIdAndTxt("005","Number of marks ~= number of spikes");
	...
	for(i=0; i<nSpikes; i++) {
		...
		if (Mark==0) {
			mexErrMsgIdAndTxt("006","CCGEngine: No zeros allowed in Marks");
			abort();
		}
	}
	...
	if (!Times || !Marks || !Count) {
		mexErrMsgIdAndTxt("007","CCGEngine could not allocate memory!\n");
	}
